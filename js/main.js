...
// ─────────────────────────────────────────────────────────────
    ...
    // STATE
    // ─────────────────────────────────────────────────────────────
    // Display Timezone feature removed — timestamps render in the
    // viewer's device timezone (DST-aware via Intl). Filing date stays UTC.
    const selectedTZ = Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC';
    ...
    const USCIS_CASE_STATUS_API_BASE = 'https://my.uscis.gov/account/case-service/api/case_status/';

    function wantCaseStatus() {
      const chk = document.getElementById('qfCaseStatusChk');
    ...
    function onQFInput() {
      ...
      if (raw.length > 2) {
        let urls = USCIS_API_BASE + raw;
        if (wantCaseStatus()) urls += '\n' + USCIS_CASE_STATUS_API_BASE + raw;
        urlDisplay.textContent = urls;
    ...
    function openUSCISAPI() {
      const raw = document.getElementById('qfReceiptInput').value.trim().toUpperCase();
      if (!raw) return;
      window.open(USCIS_API_BASE + raw, '_blank', 'noopener,noreferrer');
      if (wantCaseStatus()) {
        // Second tab: case_status endpoint — its response includes the
        // processing service center. May require allowing pop-ups.
        window.open(USCIS_CASE_STATUS_API_BASE + raw, '_blank', 'noopener,noreferrer');
      }
    }
